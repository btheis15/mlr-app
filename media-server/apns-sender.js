...
async function start() {
  ...
  // Realtime-only delivery (no sweep backstop), so a silently-dropped channel
  // stops iOS push until the mini restarts. Resubscribe on error/timeout/closed,
  // matching alert-mailer.js / push-sender.js.
  ...
  function subscribeRealtime() {
    ...
    realtimeChannel = sb.channel("apns-sender")
      ...
      .subscribe((status) => {
        ...
        if (status === "SUBSCRIBED") console.log("[apns] listening (committee + house chat + alerts + feed notifications + verified new members)");
