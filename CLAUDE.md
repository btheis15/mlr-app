...
- **Hide who's signed up (migration 0167, schedule events only — not mirrored
  ...
  - **The organizer defaults to hiding it from THEMSELVES too.** RLS lets a
    ...
    suppressing manager's own row(s) (filtered client-side out of their
    otherwise-full fetch — matched by `user_id` **or `added_by`**, so a
    hardcoded/typed-in name the manager personally added for someone without
    an account shows too, not just their own linked entry) both still show,
    with a "+N more hidden…" note for everyone else — seeing what you
    signed up or entered was never the spoiler, only seeing who else did.
...
- **Any signed-in member can create an event** (migration 0187 — the
  ...
  each row showing its urgency chip and scope (Around the Resort vs. a named
  house) — (multi-select, `add_work_item_to_event` per pick), with a "＋ Create
  a new item instead" escape hatch into the original `WorkItemComposer` flow —
