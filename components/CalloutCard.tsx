 ...
 * spotlight in HomeSpotlight, and the live preview inside the Planner's
 * Callouts sheet. Every part is optional: an image flyer can stand alone, or a
 * title/body can run without art. The action button takes any tel:/mailto:/
 * https href; a tel: link also shows the number, mirroring the old hard-coded
 * "Call Tricia" t-shirt card this replaced.
 */
export function CalloutCard({ callout }: { callout: HomeCallout }) {
  const { title, body, imageUrl, linkHref, linkLabel, endsOn } = callout;
  const href = linkHref?.trim() || null;
  const tel = href?.startsWith("tel:") ? href.slice(4) : null;
  const external = href ? /^https?:/i.test(href) : false;
  const hasText = Boolean(title?.trim() || body?.trim() || href || endsOn);
  ...
  return (
    <div className="overflow-hidden rounded-2xl bg-card ring-1 ring-border shadow-sm">
      {imageUrl && (
        ...
        // PostsView / the Planner's ImageRow render mini-hosted images.
      ...
      {hasText && (
        <div className={`px-3.5 pb-3.5 ${imageUrl ? "pt-3" : "pt-3.5"}`}>
          ...
          {href && (
            ...
            >
              <span>{linkLabel?.trim() || (tel ? "📞 Call" : external ? "Open link" : "✉️ Email")}</span>
