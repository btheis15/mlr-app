  ...
  return wrap(`${members.length} ${plural(members.length, "member")}`, (
    <>
      ...
      <div className="shrink-0 border-t border-border bg-card" style={embedded ? undefined : { paddingBottom: "env(safe-area-inset-bottom)" }}>
        ...
        {/* ── The attach button ─────────────────────────────────────────────
            ONE "+" that opens the picker DIRECTLY — its onClick is nothing but
            `fileRef.current?.click()`, and the input is a plain hidden sibling
            right next to it. Deliberately the exact shape of the Main Feed's
            post composer (PostsView), the one media picker in this app that has
            always worked in the installed iOS PWA.
            ...
            the composer, with no error. Three attempts to keep the menu and fix
            it around the edges all failed on-device (inputs mounted outside the
            menu; closing the menu only after the picker resolved; sr-only over
            display:none) — so the POPUP is gone, not the "+". A popup is not a
            safe place to trigger a file input from on iOS; the glyph was never
            the problem.
