-- 0197_event_message_guest_emails.sql
-- Fold a manually-added guest's optional email (migration 0196,
-- event_attendance.guest_email) into the event email's recipients — so a
-- guest who left an email gets kept in the loop about updates to THIS event
-- specifically (new tasks, notes, etc.), the same way an unlinked
-- family_roster person already does.
--
-- Guests always land in the GENERAL bucket, never a house bucket — a guest
-- isn't a member of any house, so there's no house-scoped list that's theirs
-- to see. Scoped inherently to one event: this whole recips CTE is keyed to
-- `m.event_id` / `p_event_id` already, so a guest's email never leaks into
-- any other event's send.
--
-- Same "not_going" exclusion as everyone else — if a manager marked a guest
-- as not coming, they don't need the work-item list either.
--
-- Return types are UNCHANGED (only the recips CTE gains a branch), so both
-- are CREATE OR REPLACE — recreated from 0193's current production bodies,
...
begin
  ...
  recips as (
    ...
    -- NEW: a manually-added guest (0196) who left an email — always general,
    -- never house-scoped.
    union all
