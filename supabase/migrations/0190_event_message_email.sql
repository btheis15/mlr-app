...
--
-- RECIPIENTS are computed here rather than reusing alert_recipients() (0127)
-- because that predates the approval gate and does not check
...
-- prevent. Otherwise the rule matches the broadcast-email doctrine: honors
-- profiles.email_alerts (this is a broadcast, not a transactional receipt —
-- see the PRINCIPLE note in CLAUDE.md's meeting-email section) and honors the
-- event-targeting rule from 0096/0127 (skip anyone who explicitly RSVP'd
-- "Can't make it"). The sender IS included, like the meeting-confirmed email —
-- it doubles as their own confirmation of what went out.
--
...
begin
  ...
  insert into public.event_messages (
    ...
    include_work_items, exclude_not_attending, recipient_count
  ) values (
...
$$;
revoke all on function public.send_event_message(text, text, text, text, text, boolean, boolean) from public, anon;
grant execute on function public.send_event_message(text, text, text, text, text, boolean, boolean) to authenticated;
