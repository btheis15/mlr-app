...
-- (0167): the aggregate is fine to reveal, the specifics aren't.
--
-- ⚠️ Gated on is_approved_member() — the same doctrine 0183 applied to every
-- other members-only read (houses, work_items, …). A SECURITY DEFINER
-- function bypasses RLS entirely, so without this check a self-signed-up but
-- not-yet-approved account (the exact threat 0181/0183 exist for) could call
-- it directly and read every house's name + item count for any event, even
-- though `houses`/`work_items` are locked to approved members. Never ship a
-- SECURITY DEFINER read without checking who's asking.
--
-- Ordered by house `position` (matching fetchHouses()) so the caller doesn't
-- have to re-sort, and marked `stable` since it's read-only. Apply after 0188.
...
begin
  ...
  return query
    ...
    group by h.id, h.name, h.emoji, h.position
